...
public class CustomerDAO {
    ...
    public List<Customer> getAllCustomers() throws SQLException {
        ...
        try (Connection conn = DatabaseConnector.getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            ...
            while (rs.next()) {
                Customer customer = new Customer(rs.getString("first_name"), rs.getString("last_name"), rs.getString("phone"), rs.getString("email"), rs.getString("client_type_name"), rs.getString("company_name"));
